...
export const useSecurePdf = () => {
   ...
   */
  const generateSessionPdf = async (sessionId: string, action: 'download' | 'print' = 'download'): Promise<void> => {
      try {
          console.log(`📄 [PDF] Demande rapport session: ${sessionId} [${action}]`);
          
          const { accessToken } = useSecureAuth();
          const config = useRuntimeConfig();
          const apiBase = config.public.apiBase || 'http://localhost:3001';
          
          const response = await fetch(`${apiBase}/api/pdf/session/${sessionId}`, {
              method: 'GET',
              headers: { 
                  'Authorization': `Bearer ${accessToken.value}`,
                  'Content-Type': 'application/json'
           ...
           const blob = await response.blob();
           let filename = `Session-${sessionId.substring(0, 8)}.pdf`;
           ...
           if (action === 'download') {
               downloadBlob(blob, filename);
               toast.add({ severity: 'success', summary: 'Succès', detail: `Rapport de session téléchargé`, life: 3000 });
           } else {
               const blobUrl = URL.createObjectURL(blob);
               window.open(blobUrl, '_blank');
           }
