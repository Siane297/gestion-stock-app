  ...
  <div class="flex flex-col gap-3">
      ...
      >
        ...
        <div class="flex justify-between items-start mb-2">
            <div>
                 ...
                 <div v-if="item.quantite_conditionnement" class="text-sm font-medium text-orange-600 bg-orange-50 px-2 py-0.5 rounded-full inline-block mt-1">
                     📦 {{ item.quantite_conditionnement }} colis ({{ item.quantite }} unités)
                 </div>
                 <div v-else class="text-sm font-medium text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full inline-block mt-1">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-gray-600 border-t pt-3 mt-1">
          ...
          <div v-if="item.prix_unitaire_conditionnement">
             ...
             <span class="font-medium font-mono text-orange-600">{{ item.prix_unitaire_conditionnement.toFixed(2) }}</span>
          </div>
    ...
    >
        <div class="flex flex-col gap-4 py-4">
            ...
            <div v-if="conditionnementOptions.length > 0" class="flex flex-col gap-2 p-3 bg-orange-50 rounded-lg border border-orange-100">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="flex flex-col gap-2">
                    <label class="font-semibold text-gray-700">
                        {{ form.conditionnement_id ? 'Nombre de Colis' : 'Quantité (Unités)' }} <span class="text-red-500">*</span>
                    </label>
                    <InputNumber
                    ...
                    <small v-if="form.conditionnement_id && selectedConditionnement" class="text-gray-500">
                        Soit <strong>{{ (form.quantite_saisie || 0) * selectedConditionnement.quantite_base }}</strong> unités en stock.
                    </small>
                </div>
            ...
            <div class="border-t pt-4 mt-2">
                <button type="button" @click="showLotDetails = !showLotDetails" class="text-sm font-medium text-primary flex items-center gap-2 mb-2">
        ...
        <template #footer>
            <div class="flex justify-end gap-2">
                ...
                <Button label="Ajouter à la commande" icon="pi pi-check" @click="submitDirect" :disabled="!isValid" />
            </div>
        </template>
    </Dialog>
  </div>
...
const conditionnementOptions = computed(() => {
    ...
    // Filtrer pour ne garder que ceux qui ne sont PAS "Unité" (qty_base > 1) ou afficher tout ?
    // L'user a demandé "acheter conditionnement", donc on affiche ceux disponibles.
    // On peut exclure l'unité de base si on veut, ou la laisser.
...
const onProductChange = () => {
    ...
    // Essayer de pré-remplir le prix unitaire si dispo (non géré ici pour l'instant, faudrait prix_achat sur produit)
    form.prix_saisie = 0; 
};

const onConditionnementChange = () => {
    // Si un conditionnement est sélectionné, on pourrait préremplir le prix du colis s'il est connu
    if (selectedConditionnement.value) {
        /*
        // Si le conditionnement a un prix d'achat défini (pas toujours le cas dans le modèle actuel qui a prix_vente)
        if (selectedConditionnement.value.prix_achat) {
             form.prix_saisie = selectedConditionnement.value.prix_achat;
        }
        */
    }
    calculateTotal();
